...
function setupConsumerProject(name: string): string {
  ...
  // Simulate node_modules/@gitlawb/ultimate-claude structure
  const pkgDir = join(tmpDir, 'node_modules', '@gitlawb', 'ultimate-claude')
...
describe('package consumer types', () => {
  test('SDK types compile for external consumer with skipLibCheck:false', () => {
    ...
    writeFileSync(
      ...
      [
        ...
        `  QueryOptions,`,
        `  SDKSession,`,
        `} from '@gitlawb/ultimate-claude/sdk'`,
        ``,
  ...
  test('SDKMessage/SDKUserMessage/SDKResultMessage are re-exported correctly', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import type { SDKMessage, SDKUserMessage, SDKResultMessage } from '@gitlawb/ultimate-claude/sdk'`,
        ``,
  ...
  test('result usage standard counters are required numbers', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import type { SDKResultMessage } from '@gitlawb/ultimate-claude/sdk'`,
        ``,
  ...
  test('SDKRateLimitError has resetsAt and rateLimitType as class properties', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import { SDKRateLimitError } from '@gitlawb/ultimate-claude/sdk'`,
        ``,
  ...
  test('control initialize response accepts generated model and account values', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import type { SDKControlInitializeResponse, ModelInfo } from '@gitlawb/ultimate-claude/sdk'`,
        ``,
