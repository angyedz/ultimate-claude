...
> {
  ...
  while (true) {
    ...
    if (canForceCompact) {
      ...
      const envSetting = process.env.ULTIMATE_CLAUDE_MAX_ACTIVE_MESSAGES
        ...
        const content =
          retryDelayMs !== undefined && retryDelayMs > 0
            ? 'The conversation is over the auto-compact threshold, but automatic compaction is cooling down after repeated failures. ' +
              'Claude Code stopped before sending another oversized request. ' +
            ...
            : 'The conversation is over the auto-compact threshold and automatic compaction has failed repeatedly. ' +
              'Claude Code stopped before sending another oversized request. Run /compact, undo recent large tool output, or start a new session with /new.'
        yield createAssistantAPIErrorMessage({
          content,
          error: 'invalid_request',
        })
