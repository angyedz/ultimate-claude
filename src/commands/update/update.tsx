...
function Update({ onDone, force, target }: UpdateProps): React.ReactNode {
  ...
  return (
    <Box flexDirection="column" marginTop={1}>
      ...
      {state.type === 'package-manager' && (
        <Box flexDirection="column" gap={1}>
          <Box>
            <StatusIcon status="warning" withSpace />
            <Text color="warning">
              Claude Code is managed by a package manager ({state.manager}).
            </Text>
          </Box>
          <Box marginLeft={2}>
            <Text dimColor>
      ...
      {state.type === 'updating' && (
        <Text color="claude">
          Updating Claude Code to {state.version} via {state.via} (this may take a
          moment)...
        </Text>
      )}
        ...
        <Box flexDirection="column" gap={1}>
          <Box>
            ...
            <Text color="success" bold>
              Claude Code updated to {state.version} via {state.via}!
            </Text>
          </Box>
          <Box marginLeft={2}>
            <Text dimColor>
...
function terminalDoneMessage(state: UpdateState): {
  ...
  switch (state.type) {
    ...
    case 'package-manager':
      return {
        message: 'Claude Code is managed by a package manager',
        delay: 3000,
      }
    case 'no-package-manager':
      return { message: 'No supported package manager found', delay: 3000 }
    case 'error':
      return { message: 'Claude Code update failed', delay: 4000 }
    default:
