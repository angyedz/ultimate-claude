...
async function withTempFile<T>(
  ...
  const dir = await mkdtemp(join(tmpdir(), 'ultimate-claude-bg-test-'))
  try {
...
describe('background session CLI parsing', () => {
  ...
  it('preserves Node exec flags and lets the launcher manage heap relaunch state', () => {
    const config = buildBackgroundChildProcessConfig({
      ...
      entrypoint: '/repo/bin/ultimate-claude',
      childArgs: ['--print', 'fix failing tests'],
      processEnv: {
        ULTIMATE_CLAUDE_HEAP_RELAUNCHED: '1',
        ULTIMATE_CLAUDE_NODE_MAX_OLD_SPACE_SIZE_MB: '8192',
      },
      sessionName: 'tests',
    ...
    expect(config.args).toEqual([
      ...
      '--expose-gc',
      '/repo/bin/ultimate-claude',
      '--print',
      'fix failing tests',
    ])
    expect(config.env.ULTIMATE_CLAUDE_HEAP_RELAUNCHED).toBeUndefined()
    expect(config.env.ULTIMATE_CLAUDE_NODE_MAX_OLD_SPACE_SIZE_MB).toBe('8192')
