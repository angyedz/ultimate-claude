...
const HEAP_RELAUNCHED_ENV = 'ULTIMATE_CLAUDE_HEAP_RELAUNCHED'
...
export async function logsHandler(
  ...
  if (!parsed.target) fail('Usage: ultimate-claude logs <id-or-name> [-f]')
...
export async function attachHandler(
  ...
  if (!target) fail('Usage: ultimate-claude attach <id-or-name>')

  await refreshBackgroundSessionStatuses()
  const session = await resolveSessionOrExit(target)
  console.error(
    `Attach is not implemented for local background sessions yet. Use \`ultimate-claude logs ${session.id} -f\` to follow output.`,
  )
  process.exitCode = 1
}
...
export async function killHandler(
  ...
  const target = normalizeArgs(args)[0]
  if (!target) fail('Usage: ultimate-claude kill <id-or-name>')
...
export async function handleBgFlag(args: string[]): Promise<void> {
  ...
  if (!parsed.prompt && !hasResumeSource(parsed.childArgs)) {
    fail('Usage: ultimate-claude --bg [--name <name>] "<prompt>"')
  }
  ...
  if (!entrypoint) {
    fail('Cannot determine Claude Code entrypoint for background session')
  }
  ...
  console.log(`Follow: ultimate-claude logs ${session.id} -f`)
  console.log(
