/**
 * `ultimate-claude auth xai ...` command handlers.
 *
...
function listenForManualCode(onLine: (line: string) => void): () => void {
  ...
  // (`ultimate-claude auth xai login`) stays alive after a successful
...
export async function xaiLogout(deps?: XaiLogoutDeps): Promise<void> {
  ...
  // xAI OAuth interactively and then ran `ultimate-claude auth xai logout`
  // ends up in a clean state. Without all four steps, the
  // marker-tagged .ultimate-claude-profile.json survives, startup
