...
describe('App stdin mode setup', () => {
  const originalDataMode = process.env.ULTIMATE_CLAUDE_USE_DATA_STDIN
  const originalReadableMode = process.env.ULTIMATE_CLAUDE_USE_READABLE_STDIN

  afterEach(() => {
    if (originalDataMode === undefined) {
      delete process.env.ULTIMATE_CLAUDE_USE_DATA_STDIN
    } else {
      process.env.ULTIMATE_CLAUDE_USE_DATA_STDIN = originalDataMode
    }
    if (originalReadableMode === undefined) {
      delete process.env.ULTIMATE_CLAUDE_USE_READABLE_STDIN
    } else {
      process.env.ULTIMATE_CLAUDE_USE_READABLE_STDIN = originalReadableMode
    }
  })
  ...
  test('resumes stdin only for opt-in data mode', () => {
    process.env.ULTIMATE_CLAUDE_USE_DATA_STDIN = '1'
    delete process.env.ULTIMATE_CLAUDE_USE_READABLE_STDIN
    const stdin = createFakeStdin()
    const app = createApp(stdin)
  ...
  test('uses data mode when ULTIMATE_CLAUDE_USE_READABLE_STDIN=0', () => {
    delete process.env.ULTIMATE_CLAUDE_USE_DATA_STDIN
    process.env.ULTIMATE_CLAUDE_USE_READABLE_STDIN = '0'
    const stdin = createFakeStdin()
    const app = createApp(stdin)
