...
function createProject(): string {
  tempDir = mkdtempSync(join(tmpdir(), 'ultimate-claude-settings-drift-'))
  return tempDir
}
...
describe('detectStaleProjectSettingsPaths', () => {
  test('warns when legacy project settings exist without canonical settings', async () => {
    ...
    expect(warning).toEqual({
      issue:
        'Legacy project settings file .claude/settings.json found, but Claude Code reads .ultimate-claude/settings.json',
      fix:
        'Move or copy .claude/settings.json to .ultimate-claude/settings.json if you intended Claude Code to use those project settings.',
    })
  })
  ...
  test('does not warn when the matching canonical project settings file exists', async () => {
    const project = createProject()
    writeJson(join(project, '.claude', 'settings.json'))
    writeJson(join(project, '.ultimate-claude', 'settings.json'))
  ...
  test('does not warn when only canonical settings files exist', async () => {
    const project = createProject()
    writeJson(join(project, '.ultimate-claude', 'settings.json'))
    writeJson(join(project, '.ultimate-claude', 'settings.local.json'))
  ...
  test('warns independently for legacy local settings', async () => {
    ...
    expect(warning?.issue).toContain('.ultimate-claude/settings.local.json')
  })
  ...
  test('warns about both legacy settings files when both canonical files are absent', async () => {
    ...
    expect(warning?.issue).toContain('.claude/settings.local.json')
    expect(warning?.issue).toContain('.ultimate-claude/settings.json')
    expect(warning?.issue).toContain('.ultimate-claude/settings.local.json')
  })
