...
describe('diagnostic redaction', () => {
  ...
  test('redacts secret-looking values even under harmless field names', () => {
    ...
    const redacted = redactDiagnosticObject({
      ...
      path: `${home}/private/ultimate-claude/src/file.ts`,
    ...
    expect(redacted.path).toBe('~/private/ultimate-claude/src/file.ts')
  ...
  test('redacts Windows-style home paths without matching sibling directories', () => {
    ...
    expect(
      redactHomePath(
        'debug path C:\\Users\\Alice\\AppData\\Roaming\\ultimate-claude',
        home,
      ),
    ).toBe('debug path ~\\AppData\\Roaming\\ultimate-claude')
    expect(redactHomePath('C:\\Users\\AliceOther\\ultimate-claude', home)).toBe(
      'C:\\Users\\AliceOther\\ultimate-claude',
    )
  })
