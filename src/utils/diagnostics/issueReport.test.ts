...
describe('diagnostic issue report', () => {
  test('builds a safe JSON report without secrets or full home paths', async () => {
    const report = await buildIssueReport({
      env: baseEnv,
      cwd: '/home/alice/private/ultimate-claude',
      ...
      errors: [
        {
          error:
            'Error: request failed with sk-openai-secret at /home/alice/private/ultimate-claude/src/file.ts',
    ...
    expect(report['ultimate-claude'].version).toBe('0.18.0')
    expect(report.workspace.cwd).toBe('ultimate-claude')
  ...
  test('formats markdown suitable for a GitHub issue', async () => {
    ...
    expect(markdown).toContain('# Claude Code diagnostic report')
  ...
  test('falls back safely when build macros are absent in source tests', async () => {
    ...
    try {
      ...
      expect(report['ultimate-claude'].version).toBe('unknown')
    } finally {
      if (hadMacro) {
  ...
  test('redacts include-debug error details', async () => {
    ...
    const report = await buildIssueReport({
      env: baseEnv,
      cwd: `${home}/private/ultimate-claude`,
      now: new Date('2026-06-15T10:30:00.000Z'),
      packageInfo: { version: '0.18.0' },
      checks: {
        buildArtifactsPresent: true,
      ...
      errors: [
        {
          error: `ProviderError: failed with sk-openai-secret-token at ${home}/private/ultimate-claude/src/file.ts`,
          timestamp: '2026-06-15T10:00:00.000Z',
        },
      ],
      includeDebug: true,
    ...
    expect(report.errors.debug).toEqual([
      'ProviderError: failed with [redacted] at ~/private/ultimate-claude/src/file.ts',
    ])
  ...
  test('writes report files and creates parent directories', () => {
    const tempDir = mkdtempSync(join(tmpdir(), 'ultimate-claude-report-'))
    try {
