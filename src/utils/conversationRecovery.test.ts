...
import { deserializeMessages as staticDeserializeMessages } from './conversationRecovery.js'
...
async function writeJsonl(entry: unknown): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'ultimate-claude-conversation-recovery-'))
  tempDirs.push(dir)
...
test('findResumeLogByPrSelector selects the first non-sidechain PR match', async () => {
  ...
  const linked = {
    ...
    sessionId: id(12),
    prNumber: 1642,
    prUrl: 'https://github.com/Gitlawb/ultimate-claude/pull/1642',
    prRepository: 'Gitlawb/ultimate-claude',
  } as any
  const sidechain = {
    ...linked,
    isSidechain: true,
  ...
  const unrelated = {
    ...
    sessionId: id(14),
    prNumber: 17,
    prUrl: 'https://github.com/Gitlawb/ultimate-claude/pull/17',
  } as any
  ...
  expect(
    findResumeLogByPrSelector(
      ...
      'https://github.com/Gitlawb/ultimate-claude/pull/1642',
    ),
  ).toBe(linked)
  expect(
...
test('deserializeMessages preserves valid legacy file and directory displayPath migration', async () => {
  ...
  const deserialized = staticDeserializeMessages([
    user(id(40), 'before'),
    attachment(
      id(41),
      {
