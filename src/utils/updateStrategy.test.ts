...
describe('isThirdPartyBuildBlockedFor', () => {
  ...
  const ULTIMATE_CLAUDE = '@gitlawb/ultimate-claude'
  ...
  test('allows a custom-PACKAGE_URL build (Claude Code) on any provider', () => {
    expect(isThirdPartyBuildBlockedFor('bedrock', ULTIMATE_CLAUDE)).toBe(false)
    expect(isThirdPartyBuildBlockedFor('firstParty', ULTIMATE_CLAUDE)).toBe(false)
  })
})
