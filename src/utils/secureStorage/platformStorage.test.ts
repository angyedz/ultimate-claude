...
describe("Secure Storage Platform Implementations", () => {
  ...
  describe("Config-Dir Isolation", () => {
    ...
    test("service name changes with ULTIMATE_CLAUDE_CONFIG_DIR", () => {
      delete process.env.ULTIMATE_CLAUDE_CONFIG_DIR;
      delete process.env.CLAUDE_CONFIG_DIR;
      const defaultName = getSecureStorageServiceName(CREDENTIALS_SERVICE_SUFFIX);

      process.env.ULTIMATE_CLAUDE_CONFIG_DIR = "/tmp/preferred-config";
      delete process.env.CLAUDE_CONFIG_DIR;
    ...
    test("Linux storage uses ULTIMATE_CLAUDE_CONFIG_DIR scoped service name", () => {
      process.env.ULTIMATE_CLAUDE_CONFIG_DIR = "/tmp/linux-preferred-scoped";
      delete process.env.CLAUDE_CONFIG_DIR;
      const expectedName = getSecureStorageServiceName(CREDENTIALS_SERVICE_SUFFIX);
    ...
    test("Windows storage uses ULTIMATE_CLAUDE_CONFIG_DIR scoped resource name", () => {
      process.env.ULTIMATE_CLAUDE_CONFIG_DIR = "/tmp/win-preferred-scoped";
      delete process.env.CLAUDE_CONFIG_DIR;
      const expectedName = getSecureStorageServiceName(CREDENTIALS_SERVICE_SUFFIX);
  ...
  describe("Windows PowerShell Escaping", () => {
    ...
    test("delete() includes legacy assembly load when explicitly enabled", () => {
      process.env.ULTIMATE_CLAUDE_ENABLE_LEGACY_WINDOWS_PASSWORDVAULT = "1";
      windowsCredentialStorage.delete();
