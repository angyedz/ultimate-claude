...
describe('KnowledgeGraph Global Persistence & RAG', () => {
  ...
  beforeEach(async () => {
    ...
    configDir = mkdtempSync(join(tmpdir(), 'ultimate-claude-test-'))
  ...
  it('persists entities across loads', async () => {
    await addGlobalEntity('tool', 'ultimate-claude', { status: 'alpha' })
    ...
    const entities = Object.values(graph.entities).filter(e => e.name === 'ultimate-claude')
  ...
  it('deduplicates entities and updates attributes', async () => {
    await addGlobalEntity('tool', 'ultimate-claude', { status: 'alpha' })
    await addGlobalEntity('tool', 'ultimate-claude', { status: 'beta', version: '0.6.0' })
