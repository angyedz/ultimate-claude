...
const originalEnv = {
  ...
  ULTIMATE_CLAUDE_DISABLE_CO_AUTHORED_BY:
    process.env.ULTIMATE_CLAUDE_DISABLE_CO_AUTHORED_BY,
...
const defaultPrAttribution =
  '🤖 Generated with [Claude Code](https://github.com/Gitlawb/ultimate-claude)'
...
beforeEach(async () => {
  ...
  delete process.env.ULTIMATE_CLAUDE_DISABLE_CO_AUTHORED_BY
...
describe('getDefaultCommitCoAuthorName', () => {
  ...
  it('uses the Claude Code email for commit attribution across providers', () => {
    expect(getDefaultCommitCoAuthorEmail('openai')).toBe(
      'ultimate-claude@gitlawb.com',
    )
...
describe('getAttributionTexts', () => {
  ...
  it('preserves includeCoAuthoredBy true as an explicit old-default opt-in', () => {
    ...
    expect(attribution.commit).toEndWith(' <ultimate-claude@gitlawb.com>')
  ...
  it('uses ULTIMATE_CLAUDE_DISABLE_CO_AUTHORED_BY to disable the old default co-author trailer', () => {
    process.env.ULTIMATE_CLAUDE_DISABLE_CO_AUTHORED_BY = '1'
    useSettings({ includeCoAuthoredBy: true })
  ...
  it('does not let ULTIMATE_CLAUDE_DISABLE_CO_AUTHORED_BY override explicit commit attribution', () => {
    process.env.ULTIMATE_CLAUDE_DISABLE_CO_AUTHORED_BY = '1'
    useSettings({
