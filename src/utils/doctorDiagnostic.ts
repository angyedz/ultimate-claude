...
export async function detectStaleProjectSettingsPaths(
  ...
  return {
    issue: `Legacy project settings file${stale.length === 1 ? '' : 's'} ${legacyPaths} found, but Claude Code reads ${canonicalPaths}`,
    fix: `Move or copy ${legacyPaths} to ${canonicalPaths} if you intended Claude Code to use those project settings.`,
  }
}
    ...
    if (!claudeInPath && !validAlias) {
      if (existingAlias) {
        ...
        warnings.push({
          ...
          fix: `Alias exists but points to invalid target: ${existingAlias}. Update alias: alias ${getCliBinaryName()}="~/.ultimate-claude/local/${getCliBinaryName()}"`,
        })
      } else {
        // No alias exists and not in PATH
        warnings.push({
          issue: 'Local installation not accessible',
          fix: `Create alias: alias ${getCliBinaryName()}="~/.ultimate-claude/local/${getCliBinaryName()}"`,
        })
      }
    }
  }
