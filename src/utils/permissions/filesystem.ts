 ...
 * .ultimate-claude/skills/{name}/ directory (global), plus the legacy global
...
export function getClaudeSkillScope(
  ...
  const bases = [
    ...
    {
      dir: expandPath(join(homedir(), '.ultimate-claude', 'skills')),
      prefix: '~/.ultimate-claude/skills/',
    },
    {
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  if (
    normalizedPath.endsWith(`${sep}.ultimate-claude${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.ultimate-claude${sep}settings.local.json`) ||
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  const openCommandsDir = join(getOriginalCwd(), '.ultimate-claude', 'commands')
  const openAgentsDir = join(getOriginalCwd(), '.ultimate-claude', 'agents')
  const openSkillsDir = join(getOriginalCwd(), '.ultimate-claude', 'skills')
...
export function isClaudeCommitMessagePath(absolutePath: string): boolean {
  const expectedPath = join(getOriginalCwd(), '.git', 'ULTIMATE_CLAUDE_COMMIT_MSG')
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  if (claudeFolderAllowRule) {
    ...
    // '~/.ultimate-claude/**', and legacy '~/.claude/**') plus narrowed skill
    // patterns like '~/.ultimate-claude/skills/my-skill/**' so users can grant
...
export function checkEditableInternalPath(
  ...
  if (
    ...
    isClaudeCommitMessagePath(normalizedPath)
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
      decisionReason: {
        type: 'other',
        reason: 'Claude Code commit message file is allowed for writing',
      },
    }
  }
