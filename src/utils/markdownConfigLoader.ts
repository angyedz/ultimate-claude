...
const PROJECT_CONFIG_DIR_NAMES = ['.claude', '.ultimate-claude'] as const
...
// ~/.ultimate-claude/agents — see issue #769) opens that many fds and blocks the
...
function recordOversizedSkip(skip: OversizedMarkdownSkip): void {
  ...
  if (!oversizedSkipStderrWarned) {
    ...
    process.stderr.write(
      `ultimate-claude: skipping oversized markdown config file ${skip.filePath} ` +
