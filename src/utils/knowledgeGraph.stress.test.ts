...
describe('KnowledgeGraph Phase 1 Stress & Edge Cases', () => {
  ...
  const originalOrama = process.env.ULTIMATE_CLAUDE_KNOWLEDGE_ORAMA
  const configDir = mkdtempSync(join(tmpdir(), 'ultimate-claude-stress-'))
  ...
  beforeEach(async () => {
    ...
    process.env.ULTIMATE_CLAUDE_KNOWLEDGE_ORAMA = '1'
  ...
  afterEach(() => {
    try {
      ...
      if (originalOrama === undefined) {
        delete process.env.ULTIMATE_CLAUDE_KNOWLEDGE_ORAMA
      } else {
        process.env.ULTIMATE_CLAUDE_KNOWLEDGE_ORAMA = originalOrama
      }
