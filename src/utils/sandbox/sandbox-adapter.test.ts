...
describe('convertToSandboxRuntimeConfig', () => {
  ...
  beforeEach(async () => {
    ...
    tempRoot = await mkdtemp(join(tmpdir(), 'ultimate-claude-sandbox-adapter-'))
  ...
  test('denies canonical Claude Code settings files in changed cwd', () => {
    const config = convertToSandboxRuntimeConfig({} as SettingsJson)

    expect(config.filesystem.denyWrite).toContain(
      resolve(activeCwd, '.ultimate-claude', 'settings.json'),
    )
    expect(config.filesystem.denyWrite).toContain(
      resolve(activeCwd, '.ultimate-claude', 'settings.local.json'),
    )
    expect(config.filesystem.denyWrite).toContain(
