 ...
 * Closes: Gitlawb/ultimate-claude#402 — JavaScript heap OOM during large tasks
 */
...
describe('cli.tsx — --provider startup ordering', () => {
  ...
  beforeEach(() => {
    clearRememberedProviderFlagForTests()
    tempDir = mkdtempSync(join(tmpdir(), 'ultimate-claude-cli-env-file-test-'))
...
describe('cli.tsx — background routing behavior', () => {
  ...
  const originalAutoRunGuard =
    process.env.ULTIMATE_CLAUDE_DISABLE_CLI_ENTRYPOINT_AUTO_RUN

  beforeAll(async () => {
    process.env.ULTIMATE_CLAUDE_DISABLE_CLI_ENTRYPOINT_AUTO_RUN = '1'
  ...
  afterAll(() => {
    if (originalAutoRunGuard === undefined) {
      delete process.env.ULTIMATE_CLAUDE_DISABLE_CLI_ENTRYPOINT_AUTO_RUN
    } else {
      process.env.ULTIMATE_CLAUDE_DISABLE_CLI_ENTRYPOINT_AUTO_RUN =
  ...
  it('treats --bg after -- as positional text, not a background flag', async () => {
    ...
    expect(mockPrintStartupScreen).toHaveBeenCalledTimes(0)
