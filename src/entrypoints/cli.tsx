...
// unexpectedly absent. Claude Code's supported runtime is Node >=22; this is
...
// Claude Code: disable experimental API betas by default.
...
export async function main(
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (based on Claude Code 0.20.1)`);
    return;
  }

  // Fast-path for `ultimate-claude ps|logs|attach|kill`.
  ...
  // Startup screen disabled — LogoV2 inside Ink UI handles branding display
  ...
  if (process.env.ULTIMATE_CLAUDE_DISABLE_EARLY_INPUT !== '1') {
    const {
...
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
if (process.env.ULTIMATE_CLAUDE_DISABLE_CLI_ENTRYPOINT_AUTO_RUN !== '1') {
  void main();
}
