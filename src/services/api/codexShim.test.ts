...
function createTempAuthJson(payload: Record<string, unknown>): string {
  const dir = mkdtempSync(join(tmpdir(), 'ultimate-claude-codex-'))
  tempDirs.push(dir)
...
describe('Codex request translation', () => {
  ...
  test('recovers Codex web search text and sources from sparse completed response', () => {
    const output = webSearchToolTest.makeOutputFromCodexWebSearchResponse(
            ...
            sources: [
              {
                ...
                url: 'https://github.com/example/ultimate-claude',
              },
            ],
          },
          {
            ...
            content: [
              {
                type: 'text',
                text: 'Claude Code is available on GitHub.',
                sources: [
                  {
                    title: 'Docs',
                    url: 'https://docs.example.com/ultimate-claude',
                  },
                ],
              },
            ],
      ...
      'Claude Code GitHub 2026',
      0.42,
    )

    expect(output.results).toEqual([
      'Claude Code is available on GitHub.',
      {
            ...
            content: [
              {
                ...
                sources: [
                  { title: 'Docs', url: 'https://docs.example.com/ultimate-claude' },
                ],
              },
            ],
          },
  ...
  test('Codex stream: tool args delivered only via function_call_arguments.done (#1259)', async () => {
    const args = '{"path":"./ultimate-claude-codex-repro","pattern":"**/*.md"}'
    const responseText = [
      'event: response.output_item.added',
