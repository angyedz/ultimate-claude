...
const ORIGINAL_TIP_EVERY = process.env.ULTIMATE_CLAUDE_ADS_TIP_EVERY
...
beforeEach(() => {
  ...
  delete process.env.ULTIMATE_CLAUDE_ADS_TIP_EVERY
})
...
afterEach(() => {
  ...
  if (ORIGINAL_TIP_EVERY === undefined) delete process.env.ULTIMATE_CLAUDE_ADS_TIP_EVERY
  else process.env.ULTIMATE_CLAUDE_ADS_TIP_EVERY = ORIGINAL_TIP_EVERY
})
...
describe('gitlawb earning tips', () => {
  ...
  test('ULTIMATE_CLAUDE_ADS_TIP_EVERY=1 shows every turn', () => {
    setAds({ enabled: true, earnCode: 'earn_abc' })
    process.env.ULTIMATE_CLAUDE_ADS_TIP_EVERY = '1'
    resetEarningCadenceForTesting()
