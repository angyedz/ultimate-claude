...
describe('getTipToShowOnSpinner — earning branch', () => {
  test('returns the Gitlawb earning tip when earning is enabled', async () => {
    ...
    const prevTipEvery = process.env.ULTIMATE_CLAUDE_ADS_TIP_EVERY
    ...
    try {
      ...
      if (prevTipEvery === undefined) delete process.env.ULTIMATE_CLAUDE_ADS_TIP_EVERY
      else process.env.ULTIMATE_CLAUDE_ADS_TIP_EVERY = prevTipEvery
    }
  })
