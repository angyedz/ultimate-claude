...
async function makeProjectDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'ultimate-claude-wiki-init-'))
  tempDirs.push(dir)
  return dir
}
...
test('initializeWiki creates the expected wiki scaffold', async () => {
  ...
  expect(result.createdFiles).toEqual([
    join('.ultimate-claude', 'wiki', 'schema.md'),
    join('.ultimate-claude', 'wiki', 'index.md'),
    join('.ultimate-claude', 'wiki', 'log.md'),
    join('.ultimate-claude', 'wiki', 'pages', 'architecture.md'),
    join('.ultimate-claude', 'wiki', 'pages', 'conventions.md'),
  ])
  ...
  expect(await readFile(paths.logFile, 'utf8')).toContain(
    'Wiki initialized by Claude Code',
  )
