...
function XaiOAuthSetup({
  ...
  return (
    <Box flexDirection="column" gap={1}>
      ...
      <Text>
        Sign in with your xAI account in the browser. Claude Code will store
      ...
      <Text dimColor>
        ...
        expected. Claude Code uses xAI's shared OAuth client.
      </Text>
...
function CodexOAuthSetup({
  ...
  return (
    <Box flexDirection="column" gap={1}>
      ...
      <Text>
        Sign in with your ChatGPT account in the browser. Claude Code will store
...
export function ProviderManager({ mode, onDone }: Props): React.ReactNode {
  ...
  function buildCodexOAuthActivationMessage(options: {
    ...
    if (options.warnings.length > 0) {
      return `${options.prefix}. Claude Code switched to it for this session with warnings: ${formatWarningsForMessage(options.warnings)}`
    }

    return `${options.prefix}. Claude Code switched to it for this session.`
  }
  ...
  function buildXaiOAuthActivationMessage(options: {
    ...
    if (options.warnings.length > 0) {
      return `${options.prefix}. Claude Code switched to it for this session with warnings: ${options.warnings.join('; ')}.`
    }
    return `${options.prefix}. Claude Code switched to it for this session.`
  }
