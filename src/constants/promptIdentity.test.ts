...
beforeAll(async () => {
  ...
  ;(globalThis as Record<string, unknown>).MACRO = {
    ...
    ISSUES_EXPLAINER:
      'report the issue at https://github.com/Gitlawb/ultimate-claude/issues',
...
test('CLI identity prefixes describe Claude Code instead of UltimateClaude', () => {
  expect(getCLISyspromptPrefix()).toContain('Claude Code')
  expect(getCLISyspromptPrefix()).not.toContain('UltimateClaude')
  ...
  for (const prefix of CLI_SYSPROMPT_PREFIXES) {
    ...
    expect(prefix).not.toContain('UltimateClaude')
    expect(prefix).not.toContain("Anthropic's official CLI for Claude")
  }
})

test('simple mode identity describes Claude Code instead of UltimateClaude', async () => {
  process.env.CLAUDE_CODE_SIMPLE = '1'

  const prompt = await getSystemPrompt([], 'gpt-4o')

  expect(prompt[0]).toContain('Claude Code')
  expect(prompt[0]).not.toContain('UltimateClaude')
...
test('built-in agent prompts describe Claude Code instead of UltimateClaude', () => {
  expect(DEFAULT_AGENT_PROMPT).toContain('Claude Code')
  expect(DEFAULT_AGENT_PROMPT).not.toContain('UltimateClaude')
  expect(DEFAULT_AGENT_PROMPT).not.toContain("Anthropic's official CLI for Claude")

  const generalPrompt = GENERAL_PURPOSE_AGENT.getSystemPrompt({
    toolUseContext: { options: {} as never },
  })
  expect(generalPrompt).toContain('Claude Code')
  expect(generalPrompt).not.toContain('UltimateClaude')
  expect(generalPrompt).not.toContain("Anthropic's official CLI for Claude")

  const explorePrompt = EXPLORE_AGENT.getSystemPrompt({
    toolUseContext: { options: {} as never },
  })
  expect(explorePrompt).toContain('Claude Code')
  expect(explorePrompt).not.toContain('UltimateClaude')
  expect(explorePrompt).not.toContain("Anthropic's official CLI for Claude")

  const planPrompt = PLAN_AGENT.getSystemPrompt({
    toolUseContext: { options: {} as never },
  })
  expect(planPrompt).toContain('Claude Code')
  expect(planPrompt).not.toContain('UltimateClaude')

  const statuslinePrompt = STATUSLINE_SETUP_AGENT.getSystemPrompt({
    toolUseContext: { options: {} as never },
  })
  expect(statuslinePrompt).toContain('Claude Code')
  expect(statuslinePrompt).not.toContain('UltimateClaude')
  ...
  expect(guidePrompt).toContain('Claude Code')
  expect(guidePrompt).toContain('You are the Claude Code guide agent.')
  expect(guidePrompt).toContain('**Claude Code** (the CLI tool)')
  expect(guidePrompt).not.toContain('You are the Claude guide agent.')
  expect(guidePrompt).not.toContain('**UltimateClaude** (the CLI tool)')
})
