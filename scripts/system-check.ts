...
export function readNodeExecutableVersion(
  ...
  if (result.error) {
    return {
      ok: false,
      detail: `Unable to run \`node --version\`: ${result.error.message}. Claude Code requires Node.js ${MIN_NODE_ENGINE_RANGE} on PATH.`,
    }
  }
  ...
  if (result.status !== 0) {
    ...
    return {
      ok: false,
      detail: `Unable to run \`node --version\`${suffix}. Claude Code requires Node.js ${MIN_NODE_ENGINE_RANGE} on PATH.`,
    }
  }
  ...
  if (!version) {
    return {
      ok: false,
      detail: `Unable to read Node.js version from \`node --version\`. Claude Code requires Node.js ${MIN_NODE_ENGINE_RANGE} on PATH.`,
    }
  }
...
async function checkBaseUrlReachability(): Promise<CheckResult> {
  ...
  try {
    ...
    if (request.transport === 'codex_responses') {
      ...
      headers.originator = 'ultimate-claude'
      method = 'POST'
      body = JSON.stringify({
