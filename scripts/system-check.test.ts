...
describe('checkNodeVersion', () => {
  ...
  test('checks the probed node executable version', () => {
    expect(checkNodeVersion({ ok: true, version: 'v20.11.1' })).toEqual({
      ...
      detail:
        'Detected 20.11.1. Claude Code requires Node.js >=22.0.0. Install Node 22 LTS or newer, then reinstall/re-run Claude Code.',
    })
  })
  ...
  test('reports a missing node executable as a Node.js version failure', () => {
    ...
    expect(checkNodeVersion(probe)).toEqual({
      ...
      label: 'Node.js version',
      detail:
        'Unable to run `node --version`: spawn node ENOENT. Claude Code requires Node.js >=22.0.0 on PATH.',
    })
  })
