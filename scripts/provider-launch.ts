...
function parseLaunchOptions(argv: string[]): LaunchOptions {
  ...
  let fast = false
  let goal = normalizeRecommendationGoal(process.env.ULTIMATE_CLAUDE_PROFILE_GOAL)
...
async function main(): Promise<void> {
  ...
  const devCode = await runProcess('node', ['bin/ultimate-claude', ...options.passthroughArgs], env)
