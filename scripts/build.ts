/**
 * Claude Code build script — bundles the TypeScript source into a single
...
result = await Bun.build({
  ...
  define: {
    ...
    'MACRO.ISSUES_EXPLAINER':
      JSON.stringify('report the issue at https://github.com/Gitlawb/ultimate-claude/issues'),
    'MACRO.FEEDBACK_CHANNEL':
      JSON.stringify('https://github.com/Gitlawb/ultimate-claude/issues'),
    'MACRO.PACKAGE_URL': JSON.stringify('@gitlawb/ultimate-claude'),
...
if (!result.success) {
  ...
  console.log(`✓ Built ultimate-claude v${version} → dist/cli.mjs`)
}
...
if (result?.success) {
  ...
  // differs only by the repo-root prefix (`/home/ubuntu/.../ultimate-claude` locally
  // vs `/home/runner/work/ultimate-claude/ultimate-claude` on CI). canonicalStub() keys
