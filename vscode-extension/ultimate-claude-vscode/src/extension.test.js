...
function createStatus(overrides = {}) {
  return {
    installed: true,
    executable: 'ultimate-claude',
    launchCommand: 'ultimate-claude --project-aware',
    terminalName: 'Claude Code',
    shimEnabled: false,
    workspaceFolder: '/workspace/ultimate-claude/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/ultimate-claude/very/long/path/example-project',
    launchCwdLabel: '/workspace/ultimate-claude/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/ultimate-claude/very/long/path/example-project/.ultimate-claude-profile.json',
    workspaceProfilePath: '/workspace/ultimate-claude/very/long/path/example-project/.ultimate-claude-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the Claude Code wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(
    html,
    /title="\/workspace\/ultimate-claude\/very\/long\/path\/example-project"[^>]*>\/workspace\/ultimate-claude\/very\/long\/path\/example-project<\//,
  );
});
...
test('UltimateClaudeControlCenterProvider.getHtml supplies a nonce to the redesigned renderer', () => {
  const { UltimateClaudeControlCenterProvider } = loadExtension();
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/ultimate-claude/src/panels/control-center.js',
      workspacePath: '/workspace/ultimate-claude',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/ultimate-claude/src/panels',
      projectAwareCwdLabel: '/workspace/ultimate-claude/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/ultimate-claude',
      workspaceRootCwdLabel: '/workspace/ultimate-claude',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/ultimate-claude',
      activeFilePath: '/workspace/ultimate-claude/src/panels/control-center.js',
      workspacePath: '/workspace/ultimate-claude',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/ultimate-claude',
      projectAwareCwdLabel: '/workspace/ultimate-claude',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/ultimate-claude/src/panels',
      launchCwdLabel: '/workspace/ultimate-claude/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/ultimate-claude\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/ultimate-claude/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/ultimate-claude',
      launchCwdLabel: '/workspace/ultimate-claude',
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/ultimate-claude/);
  assert.match(html, /Same workspace-root target as Launch Claude Code because the relative command resolves from the workspace root · \/workspace\/ultimate-claude/);
});
