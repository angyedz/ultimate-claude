 ...
 *   ~/.ultimate-claude/projects/<sanitized-cwd>/<sessionId>.jsonl
 *
...
function resolveConfigDir() {
  ...
  const ultimateClaudeDir = path.join(home, '.ultimate-claude');
  const legacyDir = path.join(home, '.claude');
  if (!fs.existsSync(ultimateClaudeDir) && fs.existsSync(legacyDir)) {
    return legacyDir;
  }
  return ultimateClaudeDir;
}
