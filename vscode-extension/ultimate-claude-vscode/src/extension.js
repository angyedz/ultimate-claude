...
const { ChatController, UltimateClaudeChatViewProvider, UltimateClaudeChatPanelManager } = require('./chat/chatProvider');
const { SessionManager } = require('./chat/sessionManager');
const { DiffContentProvider, SCHEME: DIFF_SCHEME } = require('./chat/diffController');

const ULTIMATE_CLAUDE_REPO_URL = 'https://github.com/Gitlawb/ultimate-claude';
const ULTIMATE_CLAUDE_SETUP_URL = 'https://github.com/Gitlawb/ultimate-claude/blob/main/README.md#quick-start';
const PROFILE_FILE_NAME = '.ultimate-claude-profile.json';
const SECRET_AZURE_API_KEY = 'ultimate-claude.azure.apiKey';
...
async function buildLaunchAzureEnv(configured) {
  ...
  if (!endpoint || !deployment) {
    void vscode.window.showWarningMessage(
      'Claude Code Azure chat is enabled but endpoint or deployment is missing. Run "Claude Code: Configure Azure / Foundry Chat" or set ultimate-claude.azure.* in settings.',
    );
    return env;
  }
  if (!apiKey) {
    void vscode.window.showWarningMessage(
      'Claude Code Azure chat is enabled but no API key is set. Use "Claude Code: Set Azure / Foundry API Key" or ultimate-claude.azure.apiKey (not recommended).',
    );
    return env;
  }
...
async function setAzureApiKey(context) {
  const key = await vscode.window.showInputBox({
    title: 'Claude Code — Azure / Foundry API key',
  ...
  void vscode.window.showInformationMessage('Claude Code Azure / Foundry API key saved to Secret Storage.');
}
...
async function clearAzureApiKey(context) {
  ...
  void vscode.window.showInformationMessage('Claude Code Azure / Foundry API key removed from Secret Storage.');
}
...
async function configureAzureChat(context) {
  const cfg = vscode.workspace.getConfiguration('ultimate-claude');
  const target = vscode.ConfigurationTarget.Global;

  const endpoint = await vscode.window.showInputBox({
    title: 'Claude Code — Azure / Foundry API endpoint',
  ...
  const apiVersion = await vscode.window.showInputBox({
    title: 'Claude Code — Azure API version',
  ...
  const deployment = await vscode.window.showInputBox({
    title: 'Claude Code — Azure deployment / model',
  ...
  void vscode.window.showInformationMessage(
    'Claude Code Azure / Foundry chat saved. Launch Claude Code from the Control Center or command palette.',
  );
}
...
async function collectControlCenterState() {
  const configured = vscode.workspace.getConfiguration('ultimate-claude');
  const launchCommand = configured.get('launchCommand', 'ultimate-claude');
  const terminalName = configured.get('terminalName', 'Claude Code');
...
async function launchUltimateClaude(options = {}) {
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `Claude Code command not found: ${executable}. Install it with: npm install -g @gitlawb/ultimate-claude@latest`,
      'Open Setup Guide',
      'Open Repository',
    );

    if (action === 'Open Setup Guide') {
      await vscode.env.openExternal(vscode.Uri.parse(ULTIMATE_CLAUDE_SETUP_URL));
    } else if (action === 'Open Repository') {
      await vscode.env.openExternal(vscode.Uri.parse(ULTIMATE_CLAUDE_REPO_URL));
    }
...
function getWorkspaceRootActionDetail(status, fallbackDetail) {
  ...
  if (status.launchActionsShareTargetReason === 'relative-launch-command') {
    return `Same workspace-root target as Launch Claude Code because the relative command resolves from the workspace root · ${status.workspaceRootCwdLabel}`;
  }
      ...
      <header class="hero">
        <div class="hero-top">
          <div class="brand">
            ...
            <div class="wordmark" aria-label="Claude Code wordmark">Open<span class="wordmark-accent">Claude</span></div>
            <div class="headline">
        ...
        <section class="support-card" aria-labelledby="quick-links-title">
          ...
          <div class="support-stack">
            ...
            <button class="support-link" id="repo" type="button">
              ...
              <span class="summary-detail">Browse the upstream Claude Code project.</span>
            </button>
            <button class="support-link" id="commands" type="button">
              <span class="support-link-label">Open Command Palette</span>
              <span class="summary-detail">Access VS Code and Claude Code commands quickly.</span>
            </button>
...
class UltimateClaudeControlCenterProvider {
  constructor() {
  ...
  async resolveWebviewView(webviewView) {
    ...
    webviewView.webview.onDidReceiveMessage(async message => {
      switch (message?.type) {
        case 'launch':
          await launchUltimateClaude();
          break;
        case 'launchRoot':
          await launchUltimateClaude({ requireWorkspace: true });
          break;
        case 'openProfile':
        ...
        case 'commands':
          ...
          break;
        case 'azureSettings':
          await vscode.commands.executeCommand('workbench.action.openSettings', 'ultimate-claude.azure');
          break;
        case 'refresh':
        default:
          break;
...
function activate(context) {
  ...
  const provider = new UltimateClaudeControlCenterProvider();
  ...
  const chatViewProvider = new UltimateClaudeChatViewProvider(chatController);
  const chatPanelManager = new UltimateClaudeChatPanelManager(chatController);
  ...
  statusBarItem.command = 'ultimate-claude.openChat';
  statusBarItem.show();
  ...
  chatController.onDidChangeState((state) => {
    switch (state) {
      case 'streaming':
        ...
        statusBarItem.tooltip = 'Claude Code is generating...';
        break;
      case 'connected':
        statusBarItem.text = '$(comment-discussion) Claude Code';
        statusBarItem.tooltip = 'Claude Code connected';
        break;
      default:
        statusBarItem.text = '$(comment-discussion) Claude Code';
        statusBarItem.tooltip = 'Open Claude Code Chat';
  ...
  // ── Existing commands ──
  const startCommand = vscode.commands.registerCommand('ultimate-claude.start', async () => {
    await launchUltimateClaude();
  });

  const startInWorkspaceRootCommand = vscode.commands.registerCommand(
    'ultimate-claude.startInWorkspaceRoot',
    async () => {
      await launchUltimateClaude({ requireWorkspace: true });
    },
  );

  const openDocsCommand = vscode.commands.registerCommand('ultimate-claude.openDocs', async () => {
    await vscode.env.openExternal(vscode.Uri.parse(ULTIMATE_CLAUDE_REPO_URL));
  });

  const openSetupDocsCommand = vscode.commands.registerCommand(
    'ultimate-claude.openSetupDocs',
    async () => {
      await vscode.env.openExternal(vscode.Uri.parse(ULTIMATE_CLAUDE_SETUP_URL));
    },
  );
  ...
  const openWorkspaceProfileCommand = vscode.commands.registerCommand(
    'ultimate-claude.openWorkspaceProfile',
    async () => {
      await openWorkspaceProfile();
    },
  );

  const openUiCommand = vscode.commands.registerCommand('ultimate-claude.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.ultimate-claude');
  });

  // ── New chat commands ──
  const newChatCommand = vscode.commands.registerCommand('ultimate-claude.newChat', () => {
    chatController.stopSession();
    chatController.broadcast({ type: 'session_cleared' });
  });

  const openChatCommand = vscode.commands.registerCommand('ultimate-claude.openChat', () => {
    chatPanelManager.openPanel();
  });

  const resumeSessionCommand = vscode.commands.registerCommand('ultimate-claude.resumeSession', async () => {
  ...
  const abortChatCommand = vscode.commands.registerCommand('ultimate-claude.abortChat', () => {
    chatController.abort();
  });

  const setAzureApiKeyCommand = vscode.commands.registerCommand('ultimate-claude.setAzureApiKey', async () => {
    await setAzureApiKey(context);
  });

  const clearAzureApiKeyCommand = vscode.commands.registerCommand('ultimate-claude.clearAzureApiKey', async () => {
    await clearAzureApiKey(context);
  });

  const configureAzureChatCommand = vscode.commands.registerCommand('ultimate-claude.configureAzureChat', async () => {
    await configureAzureChat(context);
  });

  const openAzureSettingsCommand = vscode.commands.registerCommand('ultimate-claude.openAzureSettings', async () => {
    await vscode.commands.executeCommand('workbench.action.openSettings', 'ultimate-claude.azure');
  });

  // ── Register providers ──
  const controlCenterProviderReg = vscode.window.registerWebviewViewProvider(
    'ultimate-claude.controlCenter',
    provider,
  );

  const chatViewProviderReg = vscode.window.registerWebviewViewProvider(
    'ultimate-claude.chat',
    chatViewProvider,
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('ultimate-claude')) {
        refreshProvider();
      }
    }),
...
module.exports = {
  activate,
  deactivate,
  UltimateClaudeControlCenterProvider,
  renderControlCenterHtml,
  resolveLaunchTargets,
  ChatController,
  UltimateClaudeChatViewProvider,
  UltimateClaudeChatPanelManager,
};
