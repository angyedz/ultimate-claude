...
test('resolveCommandCheckPath resolves workspace-relative executables', () => {
  assert.equal(
    resolveCommandCheckPath('./node_modules/.bin/ultimate-claude', '/repo'),
    require('node:path').resolve('/repo', './node_modules/.bin/ultimate-claude'),
  );
});

test('resolveCommandCheckPath leaves bare commands alone', () => {
  assert.equal(resolveCommandCheckPath('ultimate-claude', '/repo'), null);
});

test('findCommandPath treats shell-like input as a literal executable name', t => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ultimate-claude-command-'));
  t.after(() => {
    fs.rmSync(tempDir, { recursive: true, force: true });
  });

  const commandName = process.platform === 'win32'
    ? 'ultimate-claude & whoami'
    : 'ultimate-claude && whoami';
