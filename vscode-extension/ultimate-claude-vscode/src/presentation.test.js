...
test('truncateMiddle keeps the profile filename visible', () => {
  ...
  assert.equal(
    truncateMiddle('/Users/example/projects/ultimate-claude/workspace/.ultimate-claude-profile.json', 40),
    '.../.ultimate-claude-profile.json',
  );
});
...
test('truncateMiddle keeps the filename visible for Windows-style paths', () => {
  ...
  assert.equal(
    truncateMiddle('C:\\Users\\example\\ultimate-claude\\workspace\\.ultimate-claude-profile.json', 40),
    '...\\.ultimate-claude-profile.json',
  );
});
...
test('buildActionModel includes workspace-profile action when a profile exists', () => {
  ...
  const model = buildActionModel({
    canLaunchInWorkspaceRoot: true,
    workspaceProfilePath: 'C:\\Users\\example\\ultimate-claude\\workspace\\.ultimate-claude-profile.json',
  });

  assert.deepEqual(model.openProfile, {
    id: 'openProfile',
    label: 'Open Workspace Profile',
    detail: 'Inspect ...\\.ultimate-claude-profile.json',
    tone: 'neutral',
    disabled: false,
  });
});
...
function createStatus(overrides = {}) {
  return {
    ...
    shimEnabled: false,
    workspaceFolder: '/workspace/ultimate-claude',
    ...
    profileStatusHint: '/workspace/ultimate-claude/.ultimate-claude-profile.json',
    workspaceProfilePath: '/workspace/ultimate-claude/.ultimate-claude-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
      ...
      rows: [
        {
          ...
          summary: 'ultimate-claude',
          detail: '/workspace/ultimate-claude · active editor workspace',
        },
        {
          key: 'profileStatus',
          label: 'Workspace profile',
          summary: 'Found',
          detail: '/workspace/ultimate-claude/.ultimate-claude-profile.json',
          tone: 'neutral',
        },
      ],
    },
...
test('buildControlCenterViewModel keeps launch command only in summary cards', () => {
  ...
  assert.deepEqual(viewModel.summaryCards.find(card => card.key === 'launchCommand'), {
    ...
    value: 'ultimate-claude --project-aware',
    detail: 'Integrated terminal: Claude Code',
  });
